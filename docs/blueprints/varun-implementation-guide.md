...
**Update:** Supabase authentication/link later succeeded and `supabase db push` found no pending migration. The blocker is now only a non-empty database URL: Vercel’s listed Production `POSTGRES_*` variables exported empty to this session, so the seed did not connect or alter data.

**Live update:** the non-empty database URL was supplied securely. Migration, 500-case/125-member seed verification, real typed-provider replay, and an isolated database-backed Preview passed. Current local candidate `cc80c4ef8583b7584598bd4945af68e27bb57695` also fixes persisted evidence reload and adds 390px/1280px plus keyboard browser coverage. Production remains unchanged. Do not claim final release completion: a final cleanup reset disconnected from the Session pooler and clean-state counts remain unconfirmed; direct Preview browser access requires Vercel SSO bypass.

**Cleanup resolution:** the next approved reset/verify and independent aggregate passed, restoring 500 clean claims/cases across 125 scopes and zero test records. Only the SSO-protected direct Preview browser/camera check and an owner-approved Production decision remain.

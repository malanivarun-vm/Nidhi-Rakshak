...
**Live update:** migration, deterministic seed, database provider replay, and protected Preview API replay later passed. The new Preview is `https://nidhi-rakshak-rdy80s979-malanivarun-7789s-projects.vercel.app`; it verifies persisted Fight/Forward/Fix resolution behavior and the repaired Unsupported evidence reload in local `cc80c4ef8583b7584598bd4945af68e27bb57695`. Do not promote: Production is unchanged, the last synthetic cleanup reset lost its Session-pooler connection, and direct Preview browser/camera verification is blocked by Vercel SSO.

**Cleanup resolution:** the subsequent deterministic reset/verify and independent count check passed, leaving 500 clean claims/cases across 125 members and no synthetic test records. Do not promote until the SSO-protected direct Preview browser/camera check is complete and the owner approves Production.

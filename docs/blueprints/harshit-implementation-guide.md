...
**Live update:** the secure database connection enabled successful seed, real-provider, persistent resolution, and isolated Preview verification. Preview Fight/Forward/Fix/Unsupported API journeys passed; `cc80c4ef8583b7584598bd4945af68e27bb57695` additionally persists Unsupported evidence across reload. Fixture verification now includes 10 Playwright tests at 390px/1280px and keyboard activation. Production remains unchanged. The reset used to remove synthetic test records lost its pooler connection; defer push/promotion until read-only clean-state counts and SSO-protected Preview browser/camera checks can be completed.

**Cleanup resolution:** the approved retry passed seed/verify and independent counts now confirm the clean 500-case/125-member dataset with zero test rows. Production remains unchanged; only direct Preview browser/camera verification behind SSO and an owner-approved Production decision remain.

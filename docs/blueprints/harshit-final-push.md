...
**Cleanup resolution:** `seed:reset` and `seed:verify` later completed. Independent counts confirm 500 claims/cases, 125 member scopes, maximum four claims per member, and zero synthetic actions, handoffs, artifacts, or uploaded-evidence rows. Production remains unchanged; direct Preview browser/camera verification is still SSO-protected and any Production deployment requires owner approval.

...
**Update:** the CLI now authenticates and is linked to `qiinbeyviqctxscryjhw`; `supabase db push` is up to date. Do not retry the seed until a non-empty database URL is available: Vercel currently exports its listed Production `POSTGRES_*` values as empty to this session. The prior seed attempt never connected and changed no data.

**Live update:** seed/verify and real-provider replay subsequently passed with a secure local connection. An isolated Preview is `READY` at `https://nidhi-rakshak-rdy80s979-malanivarun-7789s-projects.vercel.app`; its protected API paths and four golden journeys passed. Local candidate `cc80c4ef8583b7584598bd4945af68e27bb57695` repairs database-backed evidence reload and expands browser evidence to keyboard activation and 390px/1280px. Production is unchanged. The final synthetic cleanup reset disconnected and pooler reads are currently stalled, so final counts and Preview browser/camera replay remain pending.

**Cleanup resolution:** a subsequent reset/verify completed and independent counts show the expected clean 500-case seed with no test actions, handoffs, artifacts, or uploaded evidence. Direct Preview browser/camera replay remains protected by Vercel SSO; Production remains unchanged pending owner decision.
